...
def env_preset(MDsimulation: PyLammps):
    ...
    # MDsimulation.units('lj')
    MDsimulation.atom_style('bond')

    MDsimulation.pair_style('dpd 1.0 1.0 20202')
    MDsimulation.bond_style('harmonic')

    MDsimulation.special_bonds('lj 1.0 1.0 1.0')
    
    MDsimulation.read_data('../lmps/nve_dpd.data')

    MDsimulation.neighbor('1.0 bin')
    MDsimulation.neigh_modify('every 1 delay 0 check no')

    MDsimulation.pair_style('dpd 1.0 1.0 20202')
    MDsimulation.pair_coeff('1 1 25.0 4.5')
    MDsimulation.pair_coeff('1 2 50.0 4.5')
    MDsimulation.pair_coeff('2 2 25.0 4.5')
    MDsimulation.bond_coeff('1 2.0 0.0')

    MDsimulation.comm_modify('mode single vel yes')
...
def execute(
    ...
    # newX, newV, exectime = compute_edsr(Lammps, SystemState, Dt, maxIter, boundary, shielding_matrix, disable_tqdm = disable_tqdm)

    (newX, newV), exectime = compute_edsr_notqdm(Lammps, SystemState, Dt, maxIter, boundary, shielding_matrix, disable_tqdm = disable_tqdm)
